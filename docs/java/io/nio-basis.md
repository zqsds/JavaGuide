...
```
什么是DMA 拷贝?
DMA（直接内存访问）是一种无需CPU直接参与的内存访问方式。在DMA传输过程中，DMA控制器掌控着总线，在CPU初始化DMA控制器并向I/O接口发出操作命令后，I/O接口会提出DMA请求。之后，DMA控制器会对DMA请求判别优先级并屏蔽位，向总线裁决逻辑提出总线请求。当CPU执行完成当前的总线周期之后会释放总线控制权，此时，总线裁决逻辑输出总线应答，表示DMA已经就绪，通过DMA控制器通知I/O接口开始DMA传输。
在DMA控制器的引导下，会在存储器和外设之间进行数据传送，在传送过程中不需要CPU的参与。当完成既定操作之后，DMA控制器释放总线控制权，并向I/O接口发出结束信号，当I/O接口收到结束信号之后，会停止I/O设备的工作并向CPU提出中断请求，使CPU从不介入状态解脱，并执行一段检查本次DMA传输操作正确性的代码。最后，CPU带着本次操作的结果以及状态继续执行原来的程序。
```
Java 对零拷贝的支持：
